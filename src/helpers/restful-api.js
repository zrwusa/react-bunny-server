...
let statusAsFunctionRestFul = {};
for (let status = 200; status < 600; status++) {
    statusAsFunctionRestFul['kick' + status] = (status >= 200 && status < 300)
        ? (ctx, data, message, des) => {
            standardResponse.commonSuccess(ctx, status, data, message, des)
        } : (ctx, errorMessage, errorDes, errorStack) => {
            standardResponse.commonError(ctx, status, errorMessage, errorDes, errorStack)
        }
}
export const restFulAPI = {
    ...statusAsFunctionRestFul,
    commonSuccess: function (ctx, status, data, message, des) {
        standardResponse.commonSuccess(ctx, status, data, message, des);
    },
    commonError: function (ctx, status, errorMessage, errorDes) {
        standardResponse.commonError(ctx, status, errorMessage, errorDes);
    },
    businessError: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 422, errorMessage, errorDes, errorStack);
    },
    // e.g
    Success: function (ctx, data, message, des) {
        standardResponse.commonSuccess(ctx, 200, data, message, des);
    },
    // e.g
    Created: function (ctx, data, message, des) {
        standardResponse.commonSuccess(ctx, 201, data, message, des);
    },
    // just notify client in processing,but not completed
    Accepted: function (ctx, data, message, des) {
        standardResponse.commonSuccess(ctx, 202, data, message, des);
    },
    ...
    SeeOther: function (ctx, data, message, des) {
        standardResponse.commonSuccess(ctx, 303, data, message, des);
    },
    // client error,but the server needs additional info to notify the client in the protocol structure
    BadRequest: function (ctx, errorMessage, errorDes, errorStack) {
    ...
    Forbidden: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 403, errorMessage, errorDes, errorStack);
    },
    NotFound: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 404, errorMessage, errorDes, errorStack);
    },
    // server is waiting request,but timeout
    RequestTimeout: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 408, errorMessage, errorDes, errorStack);
    },
    // e.g. sign up user info already exists
    Conflict: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 409, errorMessage, errorDes, errorStack);
    },
    // e.g. business logic error
    UnprocessableEntity: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 422, errorMessage, errorDes, errorStack);
    },
    // e.g. someone is modifying the entity witch your are accessing
    Locked: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 423, errorMessage, errorDes, errorStack);
    },
    // too many requests,the server can not process now
    TooManyRequests: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 429, errorMessage, errorDes, errorStack);
    },
    // need to be implemented
    NotImplemented: function (ctx, errorMessage, errorDes, errorStack) {
        standardResponse.commonError(ctx, 501, errorMessage, errorDes, errorStack);
    }
}
